	...
	if (_tcslen(szName)) {
		...
		if (_tcscmp(szName, _T("-listextrainfo")) == 0) {
			...
			for (i = 0; i < nBurnDrvCount; i++) {
				...
				printf("%s\t%ix%i\t%i:%i\t0x%08X\t\"%s\"\t%i\t%i\t\"%s\"\n", BurnDrvGetTextA(DRV_NAME), nWidth, nHeight, nAspectX, nAspectY, BurnDrvGetHardwareCode(), BurnDrvGetTextA(DRV_SYSTEM), BurnDrvIsWorking(), BurnDrvGetMaxPlayers(), BurnDrvGetTextA(DRV_COMMENT));
			}			
			return 1;
		}
	}
