...
#define _INDEXTOSTATEIMAGEMASK(i) ((i) << 12)
...
#define MASKALL		(MASKCPS | MASKCPS2 | MASKCPS3 | MASKNEOGEO | MASKSEGA | MASKTOAPLAN | MASKCAVE | MASKPGM | MASKTAITO | MASKPSIKYO | MASKKANEKO16 | MASKMEGADRIVE | MASKMISCPRE90S | MASKMISCPOST90S)
...
#define HORIZONTAL_MAX_SIZE	230
#define VERTICAL_MAX_SIZE	230
...
#if defined (_UNICODE)
static int UpdatePreview(bool bReset, wchar_t *szPreviewDir)
#else
static int UpdatePreview(bool bReset, char *szPreviewDir)
#endif
{
	static int nIndex;
	int nOldIndex;
...
static void CreateFilters()
{
	...
	TvItem.item.pszText = _T("Filters");
	HTREEITEM hRoot = (HTREEITEM)SendMessage(GetDlgItem(hSelDlg, IDC_TREE2), TVM_INSERTITEM, 0, (LPARAM)&TvItem);
		
	TvItem.hParent = hRoot;
	TvItem.item.pszText = _T("Hardware");
	HTREEITEM hHardware = (HTREEITEM)SendMessage(GetDlgItem(hSelDlg, IDC_TREE2), TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("Cave");
	hFilterCave = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterCave, (nLoadMenuShowX & MASKCAVE) ? FALSE : TRUE);
		
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("CPS-1");
	hFilterCps1 = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterCps1, (nLoadMenuShowX & MASKCPS) ? FALSE : TRUE);
	
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("CPS-2");
	hFilterCps2 = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterCps2, (nLoadMenuShowX & MASKCPS2) ? FALSE : TRUE);
	
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("CPS-3");
	hFilterCps3 = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterCps3, (nLoadMenuShowX & MASKCPS3) ? FALSE : TRUE);
	
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("Kaneko 16");
	hFilterKaneko16 = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterKaneko16, (nLoadMenuShowX & MASKKANEKO16) ? FALSE : TRUE);
	
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("Neo Geo");
	hFilterNeogeo = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterNeogeo, (nLoadMenuShowX & MASKNEOGEO) ? FALSE : TRUE);
	
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("PGM");
	hFilterPgm = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterPgm, (nLoadMenuShowX & MASKPGM) ? FALSE : TRUE);
	
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("Psikyo");
	hFilterPsikyo = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterPsikyo, (nLoadMenuShowX & MASKPSIKYO) ? FALSE : TRUE);
	
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("Sega");
	hFilterSega = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterSega, (nLoadMenuShowX & MASKSEGA) ? FALSE : TRUE);
	
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("Taito");
	hFilterTaito = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterTaito, (nLoadMenuShowX & MASKTAITO) ? FALSE : TRUE);
	
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("Toaplan");
	hFilterToaplan = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterToaplan, (nLoadMenuShowX & MASKTOAPLAN) ? FALSE : TRUE);
	
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("Misc (pre 90s)");
	hFilterMiscPre90s = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterMiscPre90s, (nLoadMenuShowX & MASKMISCPRE90S) ? FALSE : TRUE);
	
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("Misc (post 90s)");
	hFilterMiscPost90s = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterMiscPost90s, (nLoadMenuShowX & MASKMISCPOST90S) ? FALSE : TRUE);
	
	TvItem.hParent = hHardware;
	TvItem.item.pszText = _T("Megadrive");
	hFilterMegadrive = (HTREEITEM)SendMessage(hFilterList, TVM_INSERTITEM, 0, (LPARAM)&TvItem);
	_TreeView_SetCheckState(hFilterList, hFilterMegadrive, (nLoadMenuShowX & MASKMEGADRIVE) ? FALSE : TRUE);
	
	SendMessage(hFilterList, TVM_EXPAND,(WPARAM)TVE_EXPAND, (LPARAM)hRoot);
...
static BOOL CALLBACK DialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (Msg == WM_TIMER) {
		UpdatePreview(false, szAppPreviewsPath);
		return 0;
	}
	...
	if (Msg == WM_NOTIFY) {
		...
		if (pNmHdr->code == TCN_SELCHANGE && LOWORD(wParam) == IDC_TREE1) {
			if (bDrvSelected == true) {
				UpdatePreview(true, szAppPreviewsPath);
			}
		...
		if (pNmHdr->code == NM_RCLICK && LOWORD(wParam) == IDC_TREE1) {
			...
			hSelectHandle = (HTREEITEM)SendMessage(hSelList, TVM_GETNEXTITEM, TVGN_CARET, ~0U);
